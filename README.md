...
- 衣装の登録（色・トーン・柄・写真。ドレス、スーツ、シャツ、ブラウス、スカート、パンツ、小物など）
- 所有衣装の検索・表示（色系統、柄、トーン、種類、タグなど）
- お気に入りコーデ（スーツ＋ワイシャツ＋ネクタイ、ブラウス＋スカート＋小物などを最大3点の「完成した装い」として保存）
- 登録アイテムからの無料コーデ提案（保存済みコーデがない場合、色・トーン・柄・季節の決定的ルールで完成候補を作成。外部API・従量課金なし）
- 写真からの衣装情報入力（端末内Canvasで色・トーン・判定可能な柄を候補入力。外部AI API・従量課金なし）
- イベントとテーマ設定
- 参加者ごとの希望を踏まえた衣装の最適化（単品で成立する衣装または完成コーデを1候補として扱う）
...
「Ensemble AI」として追加した機能、期間前から存在した機能との区別、GPT-5.6 Sol / LunaとCodexの役割、構成、テスト、デモ、プライバシー方針は [`BUILD_WEEK_2026.md`](./BUILD_WEEK_2026.md) に記録しています。日本語ナレーション・英語字幕付きの提出動画と再生成手順は[`BUILD_WEEK_VIDEO.md`](./BUILD_WEEK_VIDEO.md)にあります。

GPT-5.6 SolはPM・相談・製品／セキュリティ判断、GPT-5.6 Lunaは実装担当、Sol / Lunaは検証・レビューとして、このCodexセッションの開発工程で使用しました。Codexでリポジトリ監査、コード編集、テスト、既存画面のブラウザ確認、Git追跡を行っています。Build Weekでは、写真入力の改善に加え、お気に入りコーデと、未設定時に登録済みの上衣・下衣・小物から作る無料の自動提案を追加しました。どちらも構成品を保った1候補として既存の決定的最適化へ渡します。新しいコーデUIの最終モバイル確認は本番公開前チェックとして残しています。
...
Worker更新後、`GET /api/health`の`apiVersion: "2026-07-18.2"`と`uploadLimits.maxOutfitComponents`を確認してからPWAを公開します。旧Workerでも単品提出は継続でき、完成コーデ提出は対応Workerへ更新するまで明示的に停止します。`2026-07-18.2`では構成リビジョンを保持し、同じ構成IDの写真だけを差し替えた場合もR2の古い写真を再利用しません。また、再提出時は参加者本人の提出から不要になった自動提案だけを、認証付き経路でR2写真とともに整理します。
