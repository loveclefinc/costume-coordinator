...
export default function FavoriteCombinationComposer({
  ...
  const [name, setName] = useState(initial?.combination.name ?? '')
  const [ownerId, setOwnerId] = useState(initial?.owner.id ?? ownerOptions[0]?.id ?? '')
  const [pieceIds, setPieceIds] = useState<string[]>(initial?.combination.pieceIds ?? [])
  ...
  return (
    <section className="favorite-combination-composer" aria-labelledby="favorite-combination-composer-title">
      <div className="favorite-combination-composer-heading">
        <div>
          <p className="favorite-combination-eyebrow">MY COORDINATE</p>
          <h2 id="favorite-combination-composer-title">
            {initial ? 'お気に入りコーデを編集' : 'お気に入りコーデを作成'}
          </h2>
          <p>スーツ、ワイシャツ、ネクタイなど、いつも一緒に使う衣装をまとめます。</p>
        </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="favorite-combination-fields">
          ...
          <div className="favorite-combination-field">
            <label htmlFor="favorite-combination-owner">主衣装</label>
            <select
        ...
        <fieldset className="favorite-combination-piece-fieldset">
          <legend>一緒に使う衣装</legend>
          <p id="favorite-combination-piece-help">
            1〜{MAX_FAVORITE_COMBINATION_PIECES}点を選択できます。ワイシャツとネクタイ類は各1点までです。
          </p>
        ...
        <div className="favorite-combination-form-actions">
          ...
          <button type="submit" className="favorite-combination-save" disabled={saving || ownerOptions.length === 0}>
            {saving ? '保存中…' : initial ? '変更を保存' : '組み合わせを保存'}
          </button>
        </div>
      </form>
    </section>
